...
const road=box(scene,0,.005,-80,21.3,.035,420,'#566a72');road.castShadow=false;
for(const side of [-1,1]){box(scene,side*11.4,-.015,-80,1.3,.07,420,'#d8c89e');box(scene,side*10.53,.034,-80,.12,.02,420,'#f1e8c9')}
const stripes=[];for(let i=0;i<52;i++)for(const lane of [0,2]){const m=box(scene,(lane-1)*5.4,.034,-i*5,.1,.018,2.4,'#eee5c8');m.castShadow=false;stripes.push({m,z:-i*5})}
...
function sync(objects,type){const alive=new Set(objects);for(const [obj,g] of active)if(g.userData.kind===type&&!alive.has(obj)){disposeModel(g);active.delete(obj)}for(const o of objects){let g=active.get(o);if(!g){g=type==='car'?vehicle(o.v):pet(o);g.userData.kind=type;if(type==='car'&&o.v.name==='Police'){g.userData.beacons=[];for(const side of [-1,1]){const lm=new T.MeshStandardMaterial({color:side<0?'#398aff':'#ff4545',emissive:side<0?'#0066ff':'#ff1111',emissiveIntensity:3});box(g,side*.35,1.85,.05,.55,.18,.28,lm);g.userData.beacons.push(lm)}box(g,0,.9,0,o.v.w*scale*1.01,.2,1,'#263c50')}if(o.breakdown){for(let i=0;i<4;i++){const cone=mesh(g,new T.ConeGeometry(.25,.65,6),mat('#ed9550'),(i-1.5)*.65,.33,o.v.h*scale/2+1+i*.5);box(g,(i-1.5)*.65,.055,o.v.h*scale/2+1+i*.5,.55,.1,.55,'#5a625c')}for(let i=0;i<3;i++)mesh(g,new T.IcosahedronGeometry(.27+i*.1,0),mat('#89928c'),.1+i*.14,2.1+i*.65,-o.v.h*scale*.25)}active.set(o,g);scene.add(g)}g.position.set((o.x-240)*scale,0,(o.y-535)*scale);if(type==='car'){g.rotation.y=(o.crossing?-o.dir*Math.PI/2:o.oncoming?Math.PI:0)+(o.turning?(o.target-o.lane)*.15:0);for(const side of [-1,1])if(g.userData['signal'+side])g.userData['signal'+side].visible=(o.breakdown||((o.warning>0||o.turning)&&((o.target>o.lane?1:-1)*(o.oncoming?-1:1))===side))&&Math.floor(frameTime*3)%2===0;if(g.userData.beacons)g.userData.beacons.forEach((m,i)=>m.emissiveIntensity=Math.floor(frameTime*7)%2===i?4:.1);for(const lamp of g.userData.lamps)lamp.emissiveIntensity=o.braking||o.breakdown?2.2:.25;}else{g.rotation.y=o.dir===1?0:Math.PI;g.visible=!o.done;g.userData.legs.forEach((leg,i)=>leg.rotation.z=o.wait>0?0:Math.sin(o.x*.3+i*Math.PI)*.45)}}}
...
for(const p of props)p.g.position.z=((p.z+travel+245)%260+260)%260-245;
for(const p of stripes)p.m.position.z=((p.z+travel+245)%260+260)%260-245;
...
camera.position.x+=(player.position.x*.22-camera.position.x)*.045;camera.lookAt(camera.position.x*.35,.2,-19);
