...
(() => {
 ...
 function angle(index,seed){if(seed!==cachedSeed){cachedSeed=seed;angles=[];prefix=[0];}while(angles.length<=index){const i=angles.length;let h=(Math.imul(i+17,374761393)^Math.imul(seed+1,668265263))>>>0;h=Math.imul(h^(h>>>13),1274126177)>>>0;const a=((h>>>16)%4!==0?0:(30+(h>>>3)%61)*((h&2)?1:-1))*Math.PI/180;angles.push(a);prefix.push(prefix.at(-1)+a);}return angles[index];}
